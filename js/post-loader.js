...
(function() {
      ...
      if (colonIndex > 0) {
        ...
        if (key === 'tags' && value.startsWith('[') && value.endsWith(']')) {
          try {
            ...
            value = value.slice(1, -1).split(',').map(function(tag) {
  ...
  function renderPostMeta(metadata) {
    ...
    if (metadata.category) {
      html += '<span class="post-meta-item">📁 ' + metadata.category + '</span>';
    }
  ...
  function renderPostTags(tags) {
    ...
    const html = tags.map(function(tag) {
      return '<a href="index.html?tag=' + encodeURIComponent(tag) + '" class="post-tag">#' + tag + '</a>';
    }).join('');
  ...
  function showError(message) {
    ...
    if (contentContainer) {
      contentContainer.innerHTML = '<div class="error-message"><p>⚠️ ' + message + '</p><a href="index.html">← 목록으로 돌아가기</a></div>';
    }
    updatePageTitle('오류');
  }
  ...
  function loadGiscus() {
    ...
    script.setAttribute('data-repo-id', 'YOUR_REPO_ID'); // Giscus 설정에서 복사
    script.setAttribute('data-category', 'General');
    script.setAttribute('data-category-id', 'YOUR_CATEGORY_ID'); // Giscus 설정에서 복사
    script.setAttribute('data-mapping', 'pathname');
    script.setAttribute('data-strict', '0');
    script.setAttribute('data-reactions-enabled', '1');
    script.setAttribute('data-emit-metadata', '1');
    script.setAttribute('data-input-position', 'top');
    script.setAttribute('data-theme', window.getCurrentTheme() === 'dark' ? 'dark' : 'light');
